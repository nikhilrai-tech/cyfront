<template>
  <div>
    <!-- <v-tabs v-model="atab" background-color="transparent" class="mb-3">
      ...
      <v-flex grow ></v-flex>
      <v-btn v-if="!$auth.user.is_staff" small color="primary" outlined to="/programs/starred">Starred</v-btn>
      <v-btn small color="warning" outlined class="ml-2" to="/programs/private">Private</v-btn>
    </v-tabs> -->
    <v-tabs v-model="atab" background-color="transparent" class="mb-3">
      <!-- <v-tab key="/programs"  to="/programs" @click="atab = '/programs'">Public</v-tab> -->
    ...
    <h4 class="headline mb-3 mt-0">Starred</h4>
...
export default {
  ...
  async asyncData({ store, $axios, app, $nuxt }) {
    ...
    try{
      const resp = await $axios.get('/app/main/fprograms/starred',)
