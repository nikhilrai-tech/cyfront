<template>
    ...
    <v-card id="report-main" ref="messagesContainer" :loading="loading"  max-height="80vh" style=" overflow-y: scroll;" @scroll="onScroll">
        ...
        <v-card-title class="d-flex justify-space-between">
            <span>
                <v-btn v-if="title" icon to="/hactivity" class="hover-trans2" ><v-icon>mdi-chevron-left</v-icon></v-btn>
        ...
        <v-timeline align-top dense >
            <v-timeline-item v-if="$auth.loggedIn ?(!$auth.user.is_staff && !$auth.user.is_staff) : false" small class="mt-n5 mb-5">
                <template #icon>
            ...
            <v-timeline-item v-if="$auth.loggedIn ? (!$auth.user.is_staff && !$auth.user.is_staff) : false" small class="mt-5 pb-0 mb-5">
                <template #icon>
                    <v-avatar size="35" color="black">
                        <v-img v-if="$auth.user.photo" :src="$auth.user.photo"></v-img>
                        <v-icon v-else size="50" class="rounded-circle"> mdi-account-circle </v-icon>
...
export default {
    ...
    async fetch() {
        ...
        try{
            const resp = await this.$axios.get('/app/main/apublicreport/'+this.title+'/', 
                )
