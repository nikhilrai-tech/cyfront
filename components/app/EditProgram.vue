        ...
        <form @submit.prevent="saveprogram">
          ...
          ></v-autocomplete>
              ...
              <v-col cols="12" sm="6" class="px-6">
                ...
                <v-text-field 
                v-model="program.low" placeholder="Low Reward" type="number" label="Low" outlined append-outer-icon="mdi-plus" prepend-icon="mdi-minus" 
                @click:append-outer="program.low += 100" @click:prepend="program.low -= 100" >
                  <template #append-outer="{ on, attrs }" >
                      <v-btn v-bind="attrs" :disabled="program.low >= 5000" icon @click="program.low += 100" v-on="on" ><v-icon>mdi-plus</v-icon></v-btn>
                  </template>
                ...
                <!-- <v-slider v-model="program.low"  step="100" min="100" max="5000" class="mt-n4" ticks ></v-slider> -->
              </v-col>
              <v-col cols="12" sm="6" class="px-6">
                <v-text-field 
                ...
                @click:append-outer="program.mid += 100" @click:prepend="program.mid -= 100" >
                  <template #append-outer="{ on, attrs }" >
                      <v-btn v-bind="attrs" :disabled="program.mid >= 5000" icon @click="program.mid += 100" v-on="on" ><v-icon>mdi-plus</v-icon></v-btn>
                  </template>
                  <template #prepend="{ on, attrs }" >
                      <v-btn v-bind="attrs" :disabled="program.mid <= 500" icon @click="program.mid -= 100" v-on="on" ><v-icon>mdi-minus</v-icon></v-btn>
                  </template>
                </v-text-field>
                <!-- <v-slider v-model="program.mid"  step="100" min="500" max="5000" ticks ></v-slider> -->
              </v-col>
              <v-col cols="12" sm="6" class="px-6">
                <v-text-field 
                ...
                @click:append-outer="program.high += 100" @click:prepend="program.high -= 100" >
                  <template #append-outer="{ on, attrs }" >
                      <v-btn v-bind="attrs" :disabled="program.high >= 5000" icon @click="program.high += 100" v-on="on" ><v-icon>mdi-plus</v-icon></v-btn>
                  </template>
                  <template #prepend="{ on, attrs }" >
                      <v-btn v-bind="attrs" :disabled="program.high <= 1000" icon @click="program.high -= 100" v-on="on" ><v-icon>mdi-minus</v-icon></v-btn>
                  </template>
                </v-text-field>
                <!-- <v-slider v-model="program.high"  step="100" min="1000" max="5000" ticks ></v-slider> -->
              </v-col>
              <v-col cols="12" sm="6" class="px-6">
                <v-text-field 
                ...
                @click:append-outer="program.critic += 100" @click:prepend="program.critic -= 100" >
                  <template #append-outer="{ on, attrs }" >
                      <v-btn v-bind="attrs" :disabled="program.critic >= 50000" icon @click="program.critic += 100" v-on="on" ><v-icon>mdi-plus</v-icon></v-btn>
                  </template>
                  <template #prepend="{ on, attrs }" >
                      <v-btn v-bind="attrs" :disabled="program.critic <= 2000" icon @click="program.critic -= 100" v-on="on" ><v-icon>mdi-minus</v-icon></v-btn>
                  </template>
                </v-text-field>
                <!-- <v-slider v-model="program.critic"  step="100" min="2000" max="50000" ticks ></v-slider> -->
              </v-col>
              <v-col cols="12" sm="3">
...
export default {
    ...
    mounted() {
      ...
      this.program = {...this.data,
        description: this.data.policy,
        low: this.data.lowreward,
        mid: this.data.midreward,
        high: this.data.highreward,
        critic: this.data.criticreward,
        // eslint-disable-next-line array-callback-return
        type: this.gettype(this.data.type)
    }
    ...
    methods: {
        async saveprogram () {
            ...
            try {
                const resp = await this.$axios.post('/app/main/program/'+this.$route.query.program, this.program
                )
                if(!resp.data.error){
                    // this.program = {...resp.data.program,
                    //     description: resp.data.program.policy,
                    //     low: resp.data.program.lowreward,
                    //     mid: resp.data.program.midreward,
                    //     high: resp.data.program.highreward,
                    //     critic: resp.data.program.criticreward,
                    //     // eslint-disable-next-line array-callback-return
                    //     type: this.gettype(resp.data.program.type)
                    // }
                    this.$router.push(`/p/edit?program=${resp.data.program.title}`)
                }
                // console.log(resp)
            } catch (e) {
                // console.log(e)
        ...
        gettype(name){
            if(name === 'Vulnerability Disclosure Program'){
                return 'VDP'
            }
            else if(name ==='Bug Bounty Program'){
                return 'BBP'
            }
            else if(name ==='Private Program'){
                return 'PRI'
            }
            else{
                return 'VDP'
